...
export function startHttpServer() {
    ...
    app.get("/agent/codex/usage", route(async (_req, res) => {
        res.setHeader("Cache-Control", "no-store");
        res.json({ ok: true, ...(await readCodexRateLimits(emit)) });
    }));
    ...
    // KK imports only images named by a completed Codex history item. The caller cannot supply a file path.
    app.get("/agent/codex/generated-image/:threadId/:itemId", route(async (req, res) => {
        const workspace = ensureSiteWorkspace(config);
        const threadId = routeParam(req.params.threadId);
        const itemId = routeParam(req.params.itemId);
        const history = await readCodexThread(emit, threadId, workspace.workspacePath);
        const message = history.messages.find(message => message.itemId === itemId || message.id === itemId);
        const detail = message?.detail as { kind?: string; status?: string; savedPath?: string } | undefined;
        if (detail?.kind !== "image" || detail.status !== "completed" || !detail.savedPath) return void res.status(404).json({ok:false,error:"completed image not found"});
        const filePath = detail.savedPath;
        if (!path.isAbsolute(filePath) || !/\.(?:png|jpe?g|webp)$/i.test(filePath)) return void res.status(400).json({ok:false,error:"invalid generated image"});
        const file = await stat(filePath);
        if (!file.isFile() || file.size < 1 || file.size > 25 * 1024 * 1024) return void res.status(413).json({ok:false,error:"invalid image size"});
        res.setHeader("Cache-Control", "no-store");
        res.type(path.extname(filePath)).send(await readFile(filePath));
    }));
