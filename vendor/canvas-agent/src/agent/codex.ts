...
export async function readCodexRateLimits(emit: AgentEmit) {
    return (await getCodexApp(emit)).readRateLimits();
}
...
export function samePath(left: string, right: string) {
    const normalize = (value: string) => {
        ...
        try { resolved = realpathSync.native(resolved); } catch { /* Missing paths still compare lexically. */ }
