...
import { assertDraftHasNoSensitiveValues, canvasSkillSource, samePath } from "./codex.js";
import { realpathSync } from "node:fs";

test("工作空间接受 Windows 包路径的原生规范形式，拒绝其他目录", () => {
    const cwd = process.cwd();
    assert.equal(samePath(cwd, realpathSync.native(cwd)), true);
    assert.equal(samePath(cwd, cwd + "/different-workspace"), false);
});
...
test("新建持久会话使用锁定版本支持的 legacy 历史模式", async () => {
    ...
    const starting = client.startThread("D:\\site");
    const request = writes.at(-1)!;
    assert.equal((request.params as Record<string, unknown>).historyMode, "legacy");
    (client as unknown as TestClient).handle({ id: request.id, result: { thread: { id: "durable-thread" } } });
    await Promise.resolve();
    const naming = writes.at(-1)!;
    assert.equal(naming.method, "thread/name/set");
    (client as unknown as TestClient).handle({ id: naming.id, result: {} });
    assert.equal((await starting).id, "durable-thread");
});
...
test("MCP 启动状态区分空对话预热与正常 turn 运行", async () => {
    ...
    await new Promise((resolve) => setImmediate(resolve));
    const naming = writes.find((item) => item.method === "thread/name/set");
    assert.ok(naming);
    testClient.handle({ id: naming.id, result: {} });
    await new Promise((resolve) => setImmediate(resolve));
