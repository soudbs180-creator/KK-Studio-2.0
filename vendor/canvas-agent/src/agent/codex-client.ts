...
export class CodexAppClient {
    ...
    async startThread(cwd?: string, permissionMode: AgentPermissionMode = "request", preheat = false) {
        ...
        let threadId = "";
        try {
            const { thread } = await this.request("thread/start", { ...threadSettings(permissionMode), ...(cwd ? { cwd } : {}), threadSource: "user", historyMode: "legacy" });
...
function codexConfig(permissionMode: AgentPermissionMode) {
    return { model_reasoning_summary: "auto", ...(permissionMode === "automatic" ? { approvals_reviewer: "auto_review" } : {}), mcp_servers: { "infinite-canvas": { command: canvasAgentMcp.command, args: canvasAgentMcp.args, env_vars: ["CANVAS_AGENT_TOKEN", "KK_AGENT_CONFIG_DIR", "USERPROFILE", "HOME"], default_tools_approval_mode: "approve", startup_timeout_sec: 20, tool_timeout_sec: 90 } } };
}
