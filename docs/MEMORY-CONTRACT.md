...
- Desktop 写入使用临时文件 + 原子替换，并在写入锁内比较上次读取的完整版本；
  Web 私有 IndexedDB 在单个读写事务内比较版本；Web 共享目录写入使用
  Web Locks 与 File System Access 独占写流。冲突时重新读取并重试，不覆盖
  其他窗口的新内容。单文件 records 上限 10000。
- 其他产品若要写同一文件，必须遵守版本比较与互斥写入协议。跨浏览器进程和
  Desktop 同时写同一目录尚未完成真实验证，不能把本契约视为外部客户端的
  写入授权或并发安全证明。
