...
独立预检发现远程 HTTP 插件执行风险，加载器已改为仅接收 HTTPS、拒绝远程自动重定向（含中间明文跳转）并停用旧版明文缓存；本地回归通过，最终 SHA 的补审待完成。Desktop 插件因严格 CSP 阻止 `blob:` 模块而尚未验收，开放任务 PLUGIN-DESKTOP-001；本次源码上传不宣称桌面插件已可用。
