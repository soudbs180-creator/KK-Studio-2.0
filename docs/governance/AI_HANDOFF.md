...
- 本轮变更与 Web/原生运行证据在 [change package](../changes/2026-09-24-ui-regression/verification.md)，42 态页面核对在 [architecture audit](../changes/2026-09-24-ui-regression/architecture-audit.md)。Web preview 固定 1423；隔离 `--data-dir` 的新 Tauri release 为 `src-tauri/target/release/kk-studio.exe`，原生加载的 JS/CSS 与当前 dist 哈希一致。完整 `npm run verify` 已通过 370 Node、317 browser；实现 `6f57e02` 已推送并建立[草稿 PR #19](https://github.com/soudbs180-creator/KK-Studio-2.0/pull/19)。独立 review 因 reviewer workspace 额度耗尽未完成。候选 `tokens.css` 未接线，291/40 外壳与 200/56 导出冲突未裁决，在线 Ardot 未回读。侧栏演示项目行须跟进 `TASK-PROJECT-SIDEBAR-001`。
