...
历史证据保留，追加新的运行结果或勘误。Playwright 截图使用 `test.info().outputPath()` 写入每项测试/重试独立的 `test-results/`，JSON reporter 写入 `test-results/browser-results.json`；CI 上传当次结果。这个临时目录会在下一次运行前清理，需要留档的证据应连同 SHA、命令、时间和结论复制到新的 `docs/evidence/<task-date>/`，不自动改写已有日期目录，也不修改旧次数/hash 制造成功。
