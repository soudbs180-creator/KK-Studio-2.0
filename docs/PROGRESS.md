...
独立预检发现远程 HTTP 插件可被替换并在应用权限下运行；已在加载器阻止明文/凭据/片段 URL、HTTPS 到明文的重定向，以及旧版明文缓存重启执行，并向用户提示信任边界。另确认 Desktop CSP 阻止当前插件 `blob:` 模块，登记 PLUGIN-DESKTOP-001；Web 插件验证不代表桌面可用。
