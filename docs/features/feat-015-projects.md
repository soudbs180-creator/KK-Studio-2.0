...
- 项目库中的真实本地项目、侧栏演示分组、项目包导入导出（ProjectPackageActions）、启动恢复。
...
- TASK-PROJECT-SIDEBAR-001：侧栏静态 `KK项目 / KK工作流` 仍是独立演示行，未绑定真实项目、分组和持久化编辑；不得把其局部菜单交互描述为真实项目管理。项目库本轮已移除无项目时的假卡，侧栏后续须接同一项目数据源。
