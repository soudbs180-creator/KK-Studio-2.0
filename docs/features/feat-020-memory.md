...
- 本地长期记忆（用户级）：对话消息规则采集 + 手动 Codex 提炼；Desktop 读写 `~/.kk-memory/memory.json`，Web 授权目录后只读参考共享文件。Web 未授权时使用可写的私有 IndexedDB，界面明示且可从共享视图切回；私有记忆不自动合并。
...
- Web 系统目录授权、Desktop 打包运行态、真实 Codex 对话记忆引用仍需真实环境验证；Web 私有记忆重置没有文件级备份。
- Desktop 文件锁与版本比较、Web 私有库事务比较已有回归；Web 共享写入因跨进程互斥缺口暂时禁用。外部客户端需先实现同一并发契约并联调。
