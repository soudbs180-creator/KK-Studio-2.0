...
- 浏览器：tests/browser/memory-settings.spec.ts（开关/共享状态/列表/提炼/注入）。
...
- 跨产品共享：Codex（桌面/Web）、豆包（Doubao Work Agent）、WorkBuddy 读写同一共享文件（见 MEMORY-CONTRACT.md）；换账号/换人时用户手动清空或重置。
- 对话发送前词法检索相关记忆并注入 [长期记忆] 块（执行器零修改）。
- 隐私：记忆仅存本机，不进 WebDAV 同步、localStorage、日志、导出包；与密钥/账号同级。
...
- 豆包 App 原生对话（不运行本仓库代码的端）不自动获得记忆，需 Agent 侧遵循契约注入。

## 变更记录

- 2026-09-21：创建卡片，状态 PROTOTYPE。
- 2026-09-24：TASK-MEMORY-001 接入本地长期记忆（用户级），状态推进 PARTIAL；真实账号绑定仍待 FEAT-017。
- 2026-09-24：TASK-MEMORY-002 改为本机共享（公共目录 ~/.kk-memory/memory.json，覆盖原隔离语义）；新增 MEMORY-CONTRACT.md 打通 Codex/豆包/WorkBuddy；记录用户决策。
