...
- 初始 base：c3ff0871b3db674e0ab073f1445879d84fee3507；已合入最新 `origin/main=3c4d012846e53095bd4f11343a1cd2ef61aa3bbd` 后的候选 head：`bb96dadc08c12a2177481d2ae1e4dda605cc77d4`。产品运行代码证据来自 8369185；其后只增加测试、治理、文档、审计脚本和证据整理，未改变生产 UI/素材实现。准确 head/tree 以 Git 与 PR 记录为准。
...
- 完整 `npm run verify` 通过：在当前候选 `bb96dadc08c12a2177481d2ae1e4dda605cc77d4` 的本地运行日志 `.tmp/verify-batched-final-2.log` 中记录 172 Node、197 browser，0 failure、0 flaky，UI121/0，lint/typecheck/format/build 通过；该次运行开始于 2026-09-21 09:30:14（Asia/Shanghai）。摘要见 `docs/evidence/2026-09-21-main-close-002/browser-summary.json`。
...
- CI follow-up：Hosted Windows runner 曾在 90 秒 seed 上限触发大库测试超时，并在 1 秒动画等待窗口出现一次 flaky；fixture 现按 8 项批次写入、仅首项保留 legacy preview、rows 只保留轻 metadata，并为 transaction abort 加显式失败；用例上限为 180 秒，共用动画 settle 窗口为 5 秒。未放宽业务断言；修复后定向 10/10 通过，当前候选本地完整 verify 172/197 通过，最终 PR CI 仍需重新验证。
...
- 第一次运行曾因其他任务占用 1423 读取到错误 bundle `index-D2PqpAea.js`，该次明确不计验收。端口释放后仅启动/关闭本任务 PID，并增加 production bundle 一致性断言。完整 verify 在 bb96dad 重跑为 172 Node、197 browser；运行 JSON 的三模式截图仍明确标注产品运行证据源为 8369185。
...
- Hosted GitHub Actions 对 42ec66a 的 verify 未启动，check annotation 明确为账号付款失败或 spending limit；这不是通过本地结果替代远端门禁的理由。PR #8 保持未合并，待用户恢复 Actions 额度后重新触发并回读。
