...
- CI follow-up：Hosted Windows runner 曾在 90 秒 seed 上限触发大库测试超时，并在 1 秒动画等待窗口出现一次 flaky；fixture 现按 8 项批次写入、仅首项保留 legacy preview、rows 只保留轻 metadata，并为 transaction abort 加显式失败；用例上限为 180 秒，共用动画 settle 窗口为 5 秒。未放宽业务断言；修复后定向 10/10 通过，最终 PR CI 需重新验证。
