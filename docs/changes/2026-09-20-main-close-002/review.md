...
- 范围：`frame-accuracy.spec.ts`、`creation-flow.spec.ts`、`asset-performance.spec.ts`、`helpers.ts` 与最终整合差异；初审基线为 5e44f6e，CI 失败复核绑定候选 42ec66a，修复后的审查结论随最终 head 更新。
...
- CI 失败复核：两次 hosted Windows verify 均在 `asset-performance.spec.ts:15` 的 `seedArchive` 首次 `page.evaluate` 处超时，后续素材测试能继续通过；另一次 663px UI 用例只在 1 秒动画 settle 窗口 flaky。原因是 fixture 同时保留 125 个 2.35MB Blob、125 个约 3.13MB legacy data URL 并以单事务写入，约 685MB 级别的准备峰值。
