...
PR #13 在 base `76339c9`、head `756e11a` 的独立审查又复现 CLI 长回复写坏持久化会话的 P1 问题：旧实现接受 200001 字符回复并报告成功，刷新后整段会话丢失。修复在 Google 会话统一保存边界校验，而非只改 CLI adapter；超限保留可恢复的 `unknown` 状态与旧消息，禁止重复提交。新增定向单元先失败后通过。修复后 head `ab6d37b` 已获独立源码和定向本地回归复核，未发现尚未解决的 P0/P1 问题；当前远端 CI 与真实 CLI 账号验证仍需分别确认，后者仍为 NOT RUN。
