...
- Location: tests\browser\ui-feature-parity.spec.ts:49:1
...
Locator:  locator('.conversation-panel').getByLabel('添加对话素材')
Expected: 0
Received: 1
Timeout:  5000ms

Call log:
  - Expect "toHaveCount" locator('.conversation-panel').getByLabel('添加对话素材') with timeout 5000ms
  - waiting for locator('.conversation-panel').getByLabel('添加对话素材')
    ...
    - main [ref=e76]:
      - generic [ref=e77]:
        ...
        - complementary [ref=e132]:
          ...
          - group "对话通道" [ref=e137]:
            - button "图片创作" [ref=e138] [cursor=pointer]
            - button "本地 Agent" [active] [pressed] [ref=e139] [cursor=pointer]
          - generic "本地 Agent 会话" [ref=e141]:
            - status [ref=e142]: Agent 尚未连接
            - paragraph [ref=e143]: 请先在连接设置中启动并连接本地 Agent。图片创作仍可单独使用。
          - generic [ref=e145]:
            - generic [ref=e146]: Agent 指令
            - textbox "Agent 指令" [ref=e147]:
              - /placeholder: 描述任务，让 Agent 读取并操作当前画布
            - generic [ref=e148]:
              - generic [ref=e149]: 模型：本地服务默认
              - generic [ref=e150]:
                - text: Agent 权限
                - combobox "Agent 权限" [ref=e151] [cursor=pointer]:
                  - option "每次询问" [selected]
                  - option "自动允许安全操作"
                  - option "全部放行"
            - paragraph [ref=e152]: 发送文字指令；图片参考请在画布中提供。此处权限仅用于本地 Agent。
            - generic [ref=e153]:
              - button "连接设置" [ref=e154] [cursor=pointer]
              - button "开启语音输入" [ref=e155] [cursor=pointer]
              - button "发送给 Agent" [disabled] [ref=e156]
          - generic [ref=e157]: 请确保授权，合法使用
```
...
```ts
  ...
  3  | 
  4  | test("plugin menu reflects installed canvas plugins and opens their manager", async ({
  5  |   page,
  6  | }) => {
  7  |   await page.goto("/");
  8  |   await page
  9  |     .locator(".start-composer")
  10 |     .getByRole("button", { name: "插件", exact: true })
  11 |     .click();
  12 |   const menu = page.getByRole("menu", { name: "选择插件" });
  13 |   await expect(menu).toContainText("已启用 4 个画布插件");
  14 |   await menu.getByRole("menuitem", { name: "管理画布插件" }).click();
  15 |   await expect(page.getByRole("heading", { name: "已安装插件" })).toBeVisible();
  16 | });
  17 | 
  18 | test("prompt library searches, previews and appends text without submitting", async ({
  19 |   page,
  20 | }) => {
  21 |   await page.route("https://raw.githubusercontent.com/**", (route) =>
  22 |     route.fulfill({
  23 |       json: [
  24 |         {
  25 |           id: "one",
  26 |           title: "光影产品",
  27 |           prompt: "柔和自然光，保留产品比例",
  28 |           tags: ["产品"],
  29 |         },
  30 |         { id: "two", title: "城市", prompt: "雨夜城市" },
  31 |       ],
  32 |     }),
  33 |   );
  34 |   await page.goto("/");
  35 |   await page.getByLabel("创作提示词").fill("现有草稿");
  36 |   await page.getByRole("button", { name: "提示词库", exact: true }).click();
  37 |   const dialog = page.getByRole("dialog", { name: "提示词库" });
  38 |   await dialog.getByRole("button", { name: "加载来源" }).click();
  39 |   await dialog.getByRole("searchbox", { name: "搜索提示词" }).fill("光影");
  40 |   await dialog.getByRole("button", { name: "光影产品", exact: true }).click();
  41 |   await dialog.getByRole("button", { name: "加入草稿" }).click();
  42 |   await expect(dialog).not.toBeVisible();
  43 |   await expect(page.getByLabel("创作提示词")).toHaveValue(
  44 |     "现有草稿\n\n柔和自然光，保留产品比例",
  45 |   );
  46 |   await expect(page.locator(".start-composer")).toBeVisible();
  47 | });
  48 | 
  49 | test("disconnected Agent has its own draft and cannot submit to image creation", async ({
  50 |   page,
  51 | }) => {
  52 |   await openWorkspace(page);
  53 |   const panel = page.locator(".conversation-panel");
  54 |   await panel.getByLabel("对话内容").fill("图片草稿");
  55 |   await panel.getByRole("button", { name: "本地 Agent", exact: true }).click();
  56 |   await expect(
  57 |     panel.getByText("Agent 尚未连接", { exact: true }),
  58 |   ).toBeVisible();
> 59 |   await expect(panel.getByLabel("添加对话素材")).toHaveCount(0);
     |                                            ^ Error: expect(locator).toHaveCount(expected) failed
  60 |   await panel.getByLabel("Agent 指令").fill("读取当前画布");
  61 |   await expect(
  62 |     panel.getByRole("button", { name: "发送给 Agent" }),
  63 |   ).toBeDisabled();
  64 |   await panel.getByRole("button", { name: "图片创作", exact: true }).click();
  65 |   await expect(panel.getByLabel("对话内容")).toHaveValue("图片草稿");
  66 |   await panel.getByRole("button", { name: "本地 Agent", exact: true }).click();
  67 |   await expect(panel.getByLabel("Agent 指令")).toHaveValue("读取当前画布");
  68 | });
  69 | 
```