...
- 2026-09-24 新 head `67ff18fb` 独立复审补充：原两项 P1 已关闭；发现重复工作项 id 使受控更新误改所有同名项（P2，当前验收阻断）。新增 TS/Rust 失败先行测试，计划级唯一性校验同步到两端；跨端项目包决定写入 [ADR-006](../../architecture/adr/ADR-006-stage-plan-package-contract.md)。修复后按新 SHA 复审。
- 2026-09-26 自查补充：阶段 `index` 与项目内计划 `id` 同样被写路径用作身份，外部快照可注入重复值。新增失败先行测试后，在 TS 归一化与 Rust 项目包校验处统一处理；Web 项目包导出保留无损校验，不能静默删去重复计划。完整回归和新 SHA 复审仍按原门禁执行。
- 2026-09-26 `36a3419` 独立复审结论 CHANGES REQUIRED：发现计划审批可绕过、未执行可完成、阶段工具 ABA、依赖图未校验四项 P1，另有包内计划归属不一致 P2。先用失败测试分别复现，再补宿主审批标记、完成条件、阶段写入预期 revision、依赖图与运行门禁、Web/Rust 项目包无损校验；更新原先断言错误行为的测试。修复后的新 SHA 仍需独立补审及完整门禁。
- 2026-09-26 `5fff9de` 独立复审结论 CHANGES REQUIRED：前五项关闭，但结果拒绝后成功工作项无法返工。新增单项返工及跨阶段依赖失效回归；宿主审批指定需返工项及可选新 prompt，重开受影响的阶段并清除旧产物引用。修复后重跑完整门禁与新 SHA 独立复审。
- 2026-09-26 `abb2bb8` 独立复审结论 CHANGES REQUIRED：结果返工本身已打通，但变更已批准的提示词未重走计划门（P1），且原提示词被覆盖后同 ID 重放不再幂等（P2）。新增 `reworkPrompt` 并同步 Web/Rust 包，prompt 更改时清除审批并重开整个阶段及下游；重跑完整门禁和新 SHA 独立复审。
