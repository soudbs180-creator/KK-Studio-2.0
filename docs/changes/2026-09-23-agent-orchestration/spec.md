...
- 宿主拒绝 `result_review` 时可指定本阶段需返工的工作项及新 prompt；省略选择表示本阶段全部工作项，空选择对非空阶段无效。选中项及其跨阶段下游依赖重置为 `queued`，清除旧 `assetId`、错误和结果摘要；已完成或待结果审批的下游阶段回到 `doing`，旧审批与迟到写入由 revision 拒绝。
- 原始 `prompt` 保持计划定义不变，新执行提示词存为可选 `reworkPrompt` 并随 Web/Rust 项目包往返，因此同 ID 同定义重放保留返工进度。计划门已批准后若返工提示词发生改变，整阶段工作项及下游依赖失效，清除 `planApprovedAt` 并回到 `plan_review`；宿主重新批准后才可执行。
- 未受影响的成功工作项与素材引用保持不变；只有宿主结果拒绝可使 `succeeded` 项返工，普通工作项更新仍不得重写成功项。返工后须重新执行成功并进入新的结果审批。
