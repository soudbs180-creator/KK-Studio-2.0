...
## 2026-09-24 并发安全修订（覆盖下方 Web 共享写入表述）

- Web 经 File System Access 授权后只读共享文件；Desktop 与 Web 没有共同的写锁，故 Web 不执行共享写入、清空、重置或自动采集。浏览器私有 IndexedDB 保留且可切回编辑。目录授权失效时可重新授权或退出共享视图。
- Desktop 重置先复制旧文件作为备份，再原子写入新文件；写入失败时 live 文件仍在。手动 Codex 提炼以本次发送返回的 `clientMessageId` 锁定回复，避免后续 turn 的内容被误收。
- 真实系统目录授权、Desktop release、模型引用与豆包/WorkBuddy 原生适配仍待验收；共享能力不因此标为已完成。
...
- 写入必须带上次读取版本，冲突时重新读取并合并重试；Desktop 另使用文件锁，Web 私有库用事务，Web 共享文件用独占写流。手动提炼仅接收本轮同线程、同 turnId 且已完成的 Codex 回复。旧候选的 `kk-studio-next/memory/default` 可校验迁入新库；损坏的旧数据不得静默清空。
