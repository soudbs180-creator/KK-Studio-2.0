...
## 2026-09-24 第二轮独立复核跟进

- 复核发现 Desktop/Web 写锁互不协调、Desktop 重置在备份后写失败会使 live 路径消失、提炼结果可能误收紧随其后的 turn，以及旧数据迁移依赖 `indexedDB.databases`。当前变更分别以 Web 共享只读、复制备份再写、`clientMessageId` 精确配对及无枚举 API 的旧库读取修复。
- 自动化为 415/415 Node、309/309 浏览器、91/91 Rust；真实系统 FSA 授权、Desktop release、模型引用和外部应用共享仍待联调。当前只允许草稿 PR，不能据此合并或把 FEAT-020 标为 REAL。

## 2026-09-24 复核勘误（覆盖下方旧候选结论）

- 独立代码评审对 `09b02c8` 指出五项：共享写入覆盖、提炼可能取旧/流式半句、损坏读取重复触发、旧 IndexedDB 未迁移、过长提炼行拖垮整批。后续提交已分别加入版本比较/写锁与重试、turnId + 完成阶段判定、稳定错误回调、一次性迁移和长度过滤；对应单元、浏览器及 Rust 测试通过。新提交仍需独立复审。
- Web FSA 的真实系统目录授权、跨浏览器进程与 Desktop 同时写入、Desktop release、真实模型引用和豆包/WorkBuddy 原生共享没有验证；继续保留 PARTIAL/NOT_VERIFIED，当前只能开草稿 PR。
...
- 真实 Codex 对话、Web 系统目录授权、Desktop 打包及豆包/WorkBuddy 原生接入均未完成；本复核不批准合并。旧结果仅作历史记录。
